...
if ( ! class_exists( 'Meu_Side_Cart_Settings' ) ) {
	...
	final class Meu_Side_Cart_Settings {
		...
		public static function get_color( $key ) {
			...
			return isset( $defaults[ $key ] ) ? $defaults[ $key ] : '#040f7c';
		}
		...
		private static function render_preview( $settings ) {
			...
			$preview_style       = sprintf( '--aurea-preview-primary:%1$s;--aurea-preview-accent:%2$s;', $primary, $accent );
		...
		private static function sanitize_value_by_key( $key, $value ) {
			...
			if ( in_array( $key, array( 'primary_color', 'accent_color' ), true ) ) {
				if ( ! is_scalar( $value ) ) {
					return $defaults[ $key ];
				}
